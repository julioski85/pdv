...
class Pos extends MY_Controller
{
    ...
    public function cancel_bill($id =null){
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
        $msg = $this->factura->cancel_bill($id);
            if ($msg) {
                if (0 == $msg['status']) {
                    unset($msg['status']);
                    $error = '';
                    ...
                    $this->session->set_flashdata('message', lang('Factura enviada'));
                }
            } 
            admin_redirect('pos/sales');
    ...
    public function email($id =null){
        ...
        $msg = $this->factura->send_email($id);
            if ($msg) {
                if (0 == $msg['status']) {
                    unset($msg['status']);
                    $error = '';
    ...
    public function facturar($id = null)
    {
        ...
        $sale = $this->pos_model->getInvoiceByID($this->input->post('sale_id'));
        $this->form_validation->set_rules('formasdepago', lang('forma_de_pago'), 'required');
        if (true == $this->form_validation->run()) {
            ...
            $forma_de_pago =$this->input->post("formasdepago");
            $metodo_de_pago =$this->input->post("metododepago");
            $uso_de_cfdi =$this->input->post("usodecfdi");

        }
        elseif ($this->input->post('facturar')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER['HTTP_REFERER']);
        }

        if (true == $this->form_validation->run() && $msg = $this->factura->createCfdiForOldSale($id,$forma_de_pago,$metodo_de_pago,$uso_de_cfdi)) {
            if ($msg) {
                if (0 == $msg['status']) {
                    unset($msg['status']);
                    $error = '';
            ...
            $this->load->view($this->theme . 'pos/facturar', $this->data);
        }
    }
 
  public function global_bill()
    {
         
        $this->form_validation->set_rules('periodicity', lang('periodicity'), 'required');
        if (true == $this->form_validation->run()) {
        ...
        if (true == $this->form_validation->run() && $msg = $this->factura->createCfdiGlobal($periodicity,$date,null)) {
            if ($msg) {
                if (0 == $msg['status']) {
                    unset($msg['status']);
                    $error = '';
            ...
            $this->load->view($this->theme . 'pos/global_bill', $this->data);
        }
    }
    ...
    public function close_register($user_id = null)
    {
        ...
        if (true == $this->form_validation->run() && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['debit_cardsales'] = $this->pos_model->getRegisterdebitCardSales($register_open_time, $user_id);
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $facturar = anchor('admin/pos/facturar/$1', '<i class="fa fa-money"></i> ' . lang('facturar'), 'data-toggle="modal" data-target="#myModal"');
        $enviar_factura = anchor('admin/pos/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_factura'));
        $cancel_bill = anchor('admin/pos/cancel_bill/$1', '<i class="fa fa-times-circle"></i> ' . lang('cancel_bill'));
        ...
        $this->load->library('datatables');
        $sale_table = $this->db->dbprefix('sales');  
        $si = "( SELECT sale_id, product_id, serial_no, GROUP_CONCAT(CONCAT({$this->db->dbprefix('sale_items')}.product_name, ' (', ROUND({$this->db->dbprefix('sale_items')}.quantity, {$this->Settings->qty_decimals}), ')') SEPARATOR '\n') as item_nane from {$this->db->dbprefix('sale_items')} ";
            $si .= " GROUP BY {$this->db->dbprefix('sale_items')}.sale_id ) FSI";
        if ($warehouse_id) {
            $this->datatables
                ->select($sale_table. ".id as id, DATE_FORMAT(".$sale_table.".date, '%Y-%m-%d %T') as date, ".$sale_table.".reference_no, biller,customer, FSI.item_nane as iname, (grand_total+COALESCE(rounding, 0)), paid, CONCAT(grand_total, '__', rounding, '__', paid) as balance, sale_status, payment_status, factura_id,companies.email as cemail,p.paid_by as paid_by")
                ->from('sales')
                ->join('companies', 'companies.id=sales.customer_id', 'left')
                ->join('payments as p', 'p.sale_id=sales.id', 'left')
                ->join($si, 'FSI.sale_id=sales.id', 'left')
                ->where($sale_table.'.warehouse_id', $warehouse_id)
                ->group_by('sales.id,p.paid_by');
        } else {
            $this->datatables
                ->select($sale_table. ".id as id, DATE_FORMAT(".$sale_table.".date, '%Y-%m-%d %T') as date, ".$sale_table.".reference_no, biller, customer, FSI.item_nane as iname, (grand_total+COALESCE(rounding, 0)), paid, CONCAT(grand_total, '__', rounding, '__', paid) as balance, sale_status, payment_status,factura_id, companies.email as cemail,p.paid_by as paid_by")
                ->from('sales')
    ...
    public function index($sid = null)
    {
        ...
        if (true == $this->form_validation->run()) {
            ...
            $metodo_de_pago =$this->input->post("metodo_de_pago");
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $real_unit_price = $this->sma->formatDecimal($_POST['real_unit_price'][$r],4);
                $unit_price = $this->sma->formatDecimal($_POST['unit_price'][$r],4);
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $product = [
                        ...
                        'net_unit_price'    => $this->sma->formatDecimal($item_net_price,4),
            ...
            $grand_total = $this->sma->formatDecimal(($total + $total_tax + $this->sma->formatDecimal($shipping,4) - $this->sma->formatDecimal($order_discount,4)), 4);
            $rounding = 0;
            ...
            if (!$suspend) {
                ...
                for ($r = 0; $r < $p; $r++) {
                    if (isset($_POST['amount'][$r]) && !empty($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) {
                        $amount = $this->sma->formatDecimal($_POST['balance_amount'][$r] > 0 ? $_POST['amount'][$r] - $_POST['balance_amount'][$r] : $_POST['amount'][$r],4);
        ...
        if (true == $this->form_validation->run() && !empty($products) && !empty($data)) {
            if ($suspend) {
                ...
                if($this->input->post("requiere_factura") == 1){
                    if($facturaResponse = $this->factura->createCfdi($biller_id,$customer_id,$products,$forma_de_pago,$metodo_de_pago,$uso_de_cfdi)){
                         if($facturaResponse['status'] == 0){
                           foreach ($facturaResponse['msg'] as $m) {
                             $msg .= '<br>' . $m;
                           }
                           $this->session->set_flashdata('error', $msg);
                           admin_redirect('pos');
                      }
                        $data["factura_id"] = $facturaResponse["id"];
                     }
                     else{
                         $this->session->set_flashdata('error', "Error al timbrar la factura porfavor revisa los datos fiscales");
                         admin_redirect('pos');
                     }
                } 
            ...
            if (($sid || $duplicate_sale) && $inv_items) {
                ...
                foreach ($inv_items as $item) {
                    ...
                    $row->price = $this->sma->formatDecimal($item->net_unit_price + $this->sma->formatDecimal($item->item_discount / $item->quantity,4),4);
                    $row->unit_price = $row->tax_method ? $item->unit_price + $this->sma->formatDecimal($item->item_discount / $item->quantity,4) + $this->sma->formatDecimal($item->item_tax / $item->quantity,4) : $item->unit_price + ($item->item_discount / $item->quantity);
    ...
    public function view($sale_id = null, $modal = null)
    {
        ...
        if($inv->factura_id){
           $this->data["sat"] = $this->factura->get_cfdi($inv->factura_id);
        }
