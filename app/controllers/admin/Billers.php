...
class Billers extends MY_Controller
{
	...
	//Variable indica si tiene credenciales
        $credenciales_sat = 0;
        if ($this->form_validation->run('companies/add') == true) {
            // Comprobar si se ha enviado un archivo
        if (isset($_FILES['clave_privada']) && 
           !empty($_FILES['clave_privada']['name']) && 
           isset($_FILES['certificado']) && 
           !empty($_FILES['certificado']['name']) ) {
            // Leer el archivo
            $clave_privada = file_get_contents($_FILES['clave_privada']['tmp_name']);
            $certificado = file_get_contents($_FILES['certificado']['tmp_name']);

            $clave_privada_base64 = base64_encode($clave_privada);
            $certificcado_base64 = base64_encode($certificado);
            $password = $this->input->post('contraseña');
            $rfc = $this->input->post("rfc");            
              // Borrando el archivo temporal
              unlink($_FILES['clave_privada']['tmp_name']);
              unlink($_FILES['certificado']['tmp_name']);


              if(!$this->factura->new_csd($rfc,$password,$certificcado_base64,$clave_privada_base64)){
                $this->session->set_flashdata('error', "Sucedio un error al intentar cargar las credenciales");
	    ...
	    if($credenciales) $this->factura->delete_csd($this->input->post('rfc'));
