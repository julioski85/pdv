    ...
    public function sale_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                ...
                } elseif ($this->input->post('form_action') == 'global_invoice') {
                    $sale_ids = $_POST['val'];
                    ...
                    log_message("ERROR","payment_form".$payment_form);
                    $msg = $this->factura->createCfdiGlobal($sale_ids,$periodicity,$months,$year,$biller_id,$payment_form);
                    if ($msg) {
                        if (0 == $msg['status']) {
                            unset($msg['status']);
                            $error = '';
                            ...
                            $this->session->set_flashdata('message',"Se genero la factura exitosamente");
                        }
                    } 
                    redirect($_SERVER['HTTP_REFERER']);
