...
class Sales_model extends CI_Model
{
    ...
    public function getInvoicesByFacturaId($factura_id)
    {
        $q = $this->db->get_where('sales', ['factura_id' => $factura_id]);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function updateFactura($id,$factura_id){
        if ($this->db->update('sales', ['factura_id' => $factura_id], ['id' => $id])) {
            return true;
        }
    return false;
    }
